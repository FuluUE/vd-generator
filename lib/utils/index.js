...
let getPaths = exports.getPaths = (_ref) => {
    ...
    if (group) {
        ...
        vdConfig = (0, _path.join)(config, group.replace(/>/g, '-') + '-' + (0, _camelCase2.default)(name) + '.json');
    } else {
        vdConfig = (0, _path.join)(config, (0, _camelCase2.default)(name) + '.json');
    }
    return { component: component, route: route, service: service, model: model, vdConfig: vdConfig };
};