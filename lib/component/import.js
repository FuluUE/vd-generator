...
exports.default = (srcProject, destProject, components) => {
    ...
    Object.keys(components).forEach(filename => {
        if (components[filename] && destComponents.indexOf(filename.toLocaleLowerCase()) === -1) {
            ...
            if (cfg.type === 1) {
                (0, _fsExtra.copySync)((0, _path.join)(conPath, cfg.name + '.js'), (0, _path.join)(destConPath, cfg.name + '.js'));
                (0, _fsExtra.copySync)((0, _path.join)(reduxPath, cfg.name + '.js'), (0, _path.join)(destReduxPath, cfg.name + '.js'));
                (0, _fsExtra.copySync)((0, _path.join)(modelPath, cfg.name + '.js'), (0, _path.join)(destModelsPath, cfg.name + '.js'));
            }
        }
    });
